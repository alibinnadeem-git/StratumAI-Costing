...
export async function GET(_req:Request,{params}:{params:Promise<{estimateId:string}>}){
  const {estimateId}=await params; const ctx=await requireTenantContext();
  const e=await db.costEstimate.findFirst({where:{id:estimateId,accountId:ctx.account.id},include:{lineItems:true,adders:true,project:true}}); if(!e) notFound();
  ...
  const rows:any[][]=[["Organization",ctx.organization.name],["Account",ctx.account.name],["Estimate",`EST-${String(e.number).padStart(4,"0")}`],["Name",e.name],["Project",e.project?.name??""],["Status",e.status],["Condition",e.condition],["Labor Rate",e.laborRate],["Overhead %",e.overheadPercent],["Profit %",e.profitMarginPercent],[],["Description","Category","Quantity","Unit","Labor Hrs/Unit Effective","Material/Unit","Labor Extended","Material Extended"]];
  ...
  return new Response(body,{headers:{"Content-Type":"text/csv; charset=utf-8","Content-Disposition":`attachment; filename="${ctx.account.slug}-estimate-${e.number}.csv"`}});
}
