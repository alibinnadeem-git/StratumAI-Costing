...
export default async function ViewerPage({params}:{params:Promise<{projectId:string}>}){
  ...
  return <div className="space-y-5"><section className="stratum-sheet"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="font-mono text-[10px] uppercase tracking-[0.08em] text-[#6D8AA0]">Spatial Viewer</p><h1 className="stratum-sheet-title">{project.name} Drawing Intelligence</h1><p className="mt-2 max-w-4xl text-sm text-[#9CB2C2]">Interactive layer visibility, drawing-revision overlays, spatial object inspection, calibration provenance, verification context, and cost traceability from the same Neon-backed takeoff model.</p></div><div className="flex flex-wrap gap-2"><Link href={`/projects/${projectId}/drawings/review`} className="btn-secondary">Review Queue</Link><Link href={`/projects/${projectId}/drawings/calibration`} className="btn-secondary">Calibration</Link><Link href={`/projects/${projectId}/drawings/revision-delta`} className="btn-secondary">Revision Delta</Link><Link href={`/projects/${projectId}/drawings`} className="btn-secondary">Workspace</Link></div></div></section><SpatialViewer revisions={JSON.parse(JSON.stringify(revisions))} layers={JSON.parse(JSON.stringify(layers))} objects={JSON.parse(JSON.stringify(objects))} costLinks={JSON.parse(JSON.stringify(costLinks))}/></div>;
}
