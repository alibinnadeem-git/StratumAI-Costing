...
export async function queueDrawingIngestionAction(projectId:string,formData:FormData){
  ...
  const detectedSheetNumber=inferSheetNumber(doc.name); const detectedSheetTitle=inferTitle(doc.name,detectedSheetNumber); const detectedRevision=revision?.revision||"0"; const discipline=disciplineFor(doc.documentType); const confidence=detectedSheetNumber?.length?.valueOf()?0.82:0.45;
  const existing=documentRevisionId?await db.$queryRawUnsafe<Array<{id:string}>>(`SELECT "id" FROM "PlanRoomDrawingIngestion" WHERE "documentRevisionId"=$1 AND "pageNumber"=$2`,documentRevisionId,pageNumber):[]; if(existing[0])throw new Error("This Plan Room revision/page is already in the drawing ingestion queue.");
  const id=randomUUID();
