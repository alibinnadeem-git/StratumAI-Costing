...
export default function ProjectTabs({
  project, role, initialRfis, initialTakeoffImports, initialRfqs, suppliers,
}: {
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400">{project.number ? `Project #${project.number}` : "Project"}</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex rounded-md border border-slate-200 bg-white p-0.5">
            {(["rfi", "rfq"] as const).map((t) => (
              <button
                key={t}
                onClick={() => setTab(t)}
                className={`rounded px-3 py-1.5 text-xs font-semibold transition ${tab === t ? "bg-slate-900 text-white" : "text-slate-500 hover:text-slate-800"}`}
              >
                {t === "rfi" ? "RFIs" : "Takeoff & RFQs"}
              </button>
            ))}
          </div>
          <Link href={`/costing/estimates/new?projectId=${project.id}`} className="rounded-md border border-signal-200 bg-signal-50 px-3 py-1.5 text-xs font-semibold text-signal-700">Cost this project</Link>
        </div>
      </div>

      {tab === "rfi" ? (
        <RFIBoard project={project} role={role} initialRfis={initialRfis} hideHeader />
      ) : (
        <RfqBoard project={project} role={role} initialTakeoffImports={initialTakeoffImports} initialRfqs={initialRfqs} suppliers={suppliers} />
      )}
    </div>
  );
}
