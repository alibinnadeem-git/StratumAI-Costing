...
export default async function ProjectDetailPage({ params }: { params: Promise<{ projectId: string }> }) {
  ...
  const [rfis, takeoffImports, rfqs, suppliers] = await Promise.all([
    db.rfi.findMany({ where: { projectId, project: { accountId: ctx.account.id } }, include: { createdBy: { select: { name: true, email: true } } }, orderBy: { number: "desc" } }),
  ...
  return <ProjectTabs project={{ id: project.id, name: project.name, number: project.number }} role={ctx.accountRole} initialRfis={JSON.parse(JSON.stringify(rfis))} initialTakeoffImports={JSON.parse(JSON.stringify(takeoffImports))} initialRfqs={JSON.parse(JSON.stringify(rfqs))} suppliers={JSON.parse(JSON.stringify(suppliers))} />;
}
