...
import { Bot, ChevronDown, Database, Globe2, Loader2, Mic, MicOff, Send, Sparkles, Volume2, VolumeX, X } from "lucide-react";

type Message = { role: "user" | "assistant"; text: string };
type JarvisCopilotProps = { organizationId: string; organizationName: string; accountId: string; accountName: string };

export default function JarvisCopilot({ organizationId, organizationName, accountId, accountName }: JarvisCopilotProps) {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { role: "assistant", text: `Jarvis online for ${organizationName} · ${accountName}. Ask what to do next, how this screen works, or how STRATUM Edge can help.` },
  ]);
  ...
  function chooseFemaleVoice() {
    ...
    const preferred = ["Samantha", "Ava", "Victoria", "Karen", "Moira", "Serena", "Zira", "Aria", "Jenny", "Google UK English Female", "Google US English"];
    return preferred.map((name) => voices.find((v) => v.name.toLowerCase().includes(name.toLowerCase()))).find(Boolean)
      || voices.find((v) => /female|woman|samantha|ava|victoria|zira|aria|jenny/i.test(v.name))
  ...
  function stopSpeaking() {
    ...
    setSpeaking(false);
    window.dispatchEvent(new Event("stratum-voice-end"));
  }
  ...
  function speak(text: string, force = false) {
    ...
    utterance.volume = 1;
    utterance.onstart = () => { setSpeaking(true); window.dispatchEvent(new Event("stratum-voice-start")); };
    utterance.onend = () => { setSpeaking(false); window.dispatchEvent(new Event("stratum-voice-end")); };
    utterance.onerror = () => { setSpeaking(false); window.dispatchEvent(new Event("stratum-voice-end")); };
